...
$sql1 ="UPDATE  `cchp`.`phoneoperator` SET  `name` =  '$a',
`address` =  '$b',
`location` =  '$c',
`p_no` =  '$d',
`e_id` =  '$e',
`username` =  '$f',
`password` =  '$g' WHERE  `phoneoperator`.`p_id` = '$z' ;"; 
	...
	{
		echo "<script>alert('Phone Operator Edite Sucefully');   window.location.href='vpho.php';</script>";
...
<html>
<head>
<title>Ho Edit Phone Operator</title>
...
<td>
	 ...
	 <tr>
		<th colspan="2"><!--<marquee behavior="alternate">--><font color="#000000" size="+3">Update Phone Operator Detail </font></th>
	</tr>
	<tr><td colspan="5">&nbsp;</td></tr>
		...
		mysql_select_db("cchp", $con);
		$i=1;
		$sql1="SELECT * FROM `cchp`.`phoneoperator` WHERE `phoneoperator`.`p_id` = $p_id;";
	 ...
	 <tr bgcolor="#F2F2F2">
     	...
     	<td width="50%"><label>
		<input type="hidden" name="text_pid" id="text_pid" value="<?php echo $p_id; ?>" />
